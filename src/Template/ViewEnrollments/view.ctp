...
<div class="enrollments view large-9 medium-8 columns content">
    <h3><?= h( $viewEnrollments->enrollments_id) ?></h3>
    <table class="vertical-table">
        <tr>
            <th scope="row"><?= __('Student') ?></th>
            <td><?= $this->Html->link($viewEnrollment->student_id, ['controller' => 'Students', 'action' =>
            'view', $viewEnrollment->student_id]) ?></td>
             </tr>
        <tr>
            <th scope="row"><?= __('Subject') ?></th>
            <td><?= $this->Html->link($viewEnrollment->subject_id, ['controller' => 'Subjects', 'action' =>
            'view', $viewEnrollment->subject_id]) ?>
            </td>
